...
To email executive summary generated by LLM analysis layer use the --email command ( works only if --analyse is used):

```bash
python src/jiopc_agent.py --config jiopc-agent.yaml --analyse --email <recipient_email_address>
